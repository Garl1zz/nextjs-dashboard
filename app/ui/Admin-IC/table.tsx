...
import DeleteButtonById from "@/app/ui/Admin-IC/DeleteButtonItem";
...
export default async function Table({
  ...
  return (
    <div className="overflow-x-auto">
      <table
        className={`min-w-full bg-white border border-gray-200 ${alice.className}`}
      >
          ...
          {searchCatalogue.map((product, index) => {
            ...
            return (
              <tr key={index} className="border-t">
                <td
                  className={`text-lg py-3 px-4 ${alice.className}`}
                >
                  {product.id_produk}
                </td>
                <td className={`text-lg py-3 px-4 ${alice.className}`}>
                  {product.name}
                </td>
                <td
                  className={`text-lg py-3 px-4 ${alice.className}`}
                ...
                >
                  <div className="flex justify-center space-x-2">
                    ...
                    <Link
                      href={`/adminpage/itemcatalogue/editproduct/${product.id_produk}`}
                    >
